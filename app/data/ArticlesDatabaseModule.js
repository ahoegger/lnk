...
function _insertArticle(article) {
    var newId;
    var clonedArticle;
    if (!(article instanceof articleModule.new)) {
        logger.error('Article ' + article + ' not of type Article');
    ...
    clonedArticle = articleModule.clone(article);
...
function _selectArticle(id) {
    ...
    return articleModule.clone(articles[position]);
}
